...
impl ArgsExtractor {
        ...
        for arg in args {
            match arg {
                Argument::Literal(lit) => match lit {
                    Literal::Identifier(name) => { ids.insert(index, name.clone()); },
                    Literal::String(s) => { ids.insert(index, s.clone()); },
                    _ => {}
                },
    ...
    pub fn extract_literal_identifiers(&self, args: &[Argument]) -> Vec<String> {
        ...
        for arg in args {
            match arg {
                Argument::Literal(lit) => match lit {
                    Literal::Identifier(name) => { ids.push(name.clone()); },
                    Literal::String(s) => { ids.push(s.clone()); },
                    _ => {}
                },
                Argument::Array(elems) => {
                    for lit in elems {
    ...
    pub fn extract_var_values_lin_expr(&self, index: usize, constraint: &Call, arrays: &HashMap<Identifier, Array>) -> Vec<Identifier> {
    ...
    pub fn extract_int_array_element(
        ...
        let idx =
            self.extract_int_value(self.extract_term(constraint, ARRAY_IDX_INDEX), solution);
    ...
    pub(crate) fn extract_int_value(
        &self,
        literal: Literal,
        solution: &HashMap<String, VariableValue>,
    ) -> i64 {
    ...
    pub(crate) fn extract_term(&self, constraint: &Call, index: usize) -> Literal {
        let arg = constraint
            .args
            .get(index)
    ...
    pub(crate) fn extract_array<'a>(&self, constraint: &Call, arrays: &'a HashMap<Identifier, Array>) -> &'a Array {
        match self.extract_term(constraint, 1) {
            Literal::Identifier(id) => {
                arrays.get(&id).unwrap_or_else(|| {
                    panic!(
