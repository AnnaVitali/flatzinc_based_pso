...
impl Normalizer {
    ...
    pub fn normalize(&self, solution: &HashMap<String, VariableValue>) -> HashMap<String, f64> {
        // Implementation for normalizing a solution
        ...
        for(var_name, var_value) in solution.iter() {
            ...
            println!("Warning: Variable {} has undefined bounds. Skipping normalization.", var_name);
            continue;
        };
            ...
            match (var_value, min_value, max_value) {
                ...
                (VariableValue::Float(value), VariableValue::Float(min), VariableValue::Float(max)) => {
    ...
    pub fn denormalize(&self, normalized_solution: &HashMap<String, f64>) -> HashMap<String, VariableValue> {
        ...
        for(var_name, normalized_value) in normalized_solution.iter() {
            let Some((min_value, max_value)) = self.bounds.get(var_name) else {
            println!("Warning: Variable {} has undefined bounds. Skipping denormalization.", var_name);
            continue;
        };
            match (min_value, max_value) {
                (VariableValue::Int(min), VariableValue::Int(max)) => {
                    ...
                    denormalized_solution.insert(var_name.clone(), VariableValue::Int(denormalized_value));
                },
                (VariableValue::Float(min), VariableValue::Float(max)) => {
                    let range = (*max - *min).max(1.0);
                    let n = (*normalized_value).min(1.0).max(0.0);
                    let denormalized_value = n * range + *min;
                    denormalized_solution.insert(var_name.clone(), VariableValue::Float(denormalized_value));
                },
                _ => {}
            }
        }
