...
impl PSOParticle {
    ...
    pub fn random_initialize_position_and_velocity(&mut self) {
        ...
        (self.local_best_obj, self.local_best_violation) = self.evaluate_current_violations(&denormalized_position);
    }
    ...
    pub fn update_velocity_and_position(
        ...
        for (var_name, var_value) in &self.position {
            ...
            if crazy_factor < 0.5{
                ...
                self.velocity.insert(var_name.clone(), crazy_vel);
            }
        }

        self.position = new_position;
        let denormalized_position = self.normalizer.denormalize(&self.position);
        let(candidate_obj, candidate_violation) = self.evaluate_current_violations(&denormalized_position);
...
impl std::fmt::Debug for BasePSO {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
