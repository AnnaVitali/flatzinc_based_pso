...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let eval_fn = |solution: &[f64]| {
        // solution[0] = x1, solution[1] = x2
        let x1 = solution[0];
        let x2 = solution[1];

        // Constraint 1: 100.0 - pow(x1 - 5.0, 2.0) - pow(x2 - 5.0, 2.0) <= 0
        let c1 = 100.0 - (x1 - 5.0).powf(2.0) - (x2 - 5.0).powf(2.0);
        // Constraint 2: pow(x1 - 6.0, 2.0) + pow(x2 - 5.0, 2.0) - 82.81 <= 0
        let c2 = (x1 - 6.0).powf(2.0) + (x2 - 5.0).powf(2.0) - 82.81;
        let violation = c1.max(0.0) + c2.max(0.0);

        // Objective: pow(x1 - 10.0, 3.0) + pow(x2 - 20.0, 3.0)
